    ...
    switch (args[0]) {
            ...
            if(i == 3) {
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        ...
                        p.push(o)
                        msgf += `${number++}. ${o.name} - Xưởng xe level ${o.Island.level}\n`
                    }
                }
     ...
     return api.sendMessage({body: `── [ Shop ] ──  \n\n🐸Danh sách xe đua bạn có thể mua\n[🏎1]. Song thần sinh đôi\n[🏎2].S-LIGHT\n[🏎3].ETARNA\n[🏎4].Sói Địa Ngục\n[🏎5].Vết Gió\n[🏎6].Tia Chóp\n[🏎7].Lamborini\n[🏎8].Hào Quang\n[🏎9].Bá Chủ\n[⭐️] Hãy reply tin nhắn bot và kèm theo số`, attachment: await this.image('https://i.imgur.com/z4Q5tWp.jpg')}, threadID, (error, info) => {
         ...
         case 'change': {
            ...
            return api.sendMessage({body:`Vui lòng nhập các lựa chọn.\n1. Đổi tiền qua coins game!\n2. Đổi coins game qua tiền\n3. Mua lượt quay!`, attachment: await this.image('https://i.imgur.com/clX87Qt.jpg')}, threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,
                    author: event.senderID,
     ...
     return api.sendMessage({body: `── [ CHỌN MAPS THI ĐẤU ] ──  \n\n❤️ Chọn maps để thi đấu\n[🗺1]. Rừng Tàn Phá\n[🗺2]. Lông Môn Tân Xuân\n[🗺3]. Ngôi Nhà Linh Thiên\n`, attachment: await this.image('https://i.imgur.com/9YaNyZA.jpg')}, threadID, (error, info) => {
        ...
        case 'top': {
            ...
            var msg = '===TOP 3 XƯỞNG XE LEVEL CAO NHẤT===\n'
            for(var i = 0; i < 3; i++) {
                msg += `${i+1}. ${p[i].name} với xưởng xe level ${p[i].Island.level}\n`
            }
  ...
  const fs = require("fs");
    switch (handleReply.type) {
        case 'build': {
            ...
            if(a.coins < a.Island.coinsLV * (a.Island.data[l[parseInt(body) - 1]] + 1)) return api.sendMessage(`Bạn không đủ số coins trong game để Nâng cấp!`, threadID, messageID);
            ...
            if(body == 1) {
                if(a.Island.data.tower == 50) return api.sendMessage('Cấp bậc động cơ này đang ở mức tối đa nên không thể Nâng cấp', threadID, messageID);
                a.Island.data.tower = a.Island.data.tower + 10;
                api.sendMessage(`Nâng cấp thành công: ${a.Island.data.tower}/50`, threadID, messageID);
            }
            if(body == 2) {
                if(a.Island.data.tree == 50) return api.sendMessage('Cấp bậc động cơ này đang ở mức tối đa nên không thể Nâng cấp', threadID, messageID);
                a.Island.data.tree = a.Island.data.tree + 10;
                api.sendMessage(`Nâng cấp thành công: ${a.Island.data.tree}/50`, threadID, messageID);
            }
            if(body == 3) {
                if(a.Island.data.pool == 50) return api.sendMessage('Cấp bậc động cơ này đang ở mức tối đa nên không thể Nâng cấp', threadID, messageID);
                a.Island.data.pool = a.Island.data.pool + 10;
                api.sendMessage(`Nâng cấp thành công: ${a.Island.data.pool}/50`, threadID, messageID);
            }
            if(body == 4) {
                if(a.Island.data.pet == 50) return api.sendMessage('Cấp bậc động cơ này đang ở mức tối đa nên không thể Nâng cấp', threadID, messageID);
            ...
            if(a.Island.data.tower == 50 && a.Island.data.tree == 50 && a.Island.data.pool == 50 && a.Island.data.pet == 50) {
                await new Promise(resolve => setTimeout(resolve, 1000));
                api.sendMessage(`Xưởng xe của bạn đã đạt được cấp tối đa!\nBạn sẽ được nâng cấp lên xưởng xe ${(a.Island.level) + 1}`, threadID, messageID);
...
Sức mạnh tông: 100%`, attachment: await this.image('https://i.imgur.com/HfK8lJD.jpg')}, threadID, messageID);
        ...
        case 'đua': {
            ...
            api.unsendMessage(handleReply.messageID)
            if(body == 1) {
  var coinduaxe = Math.floor(Math.random() * 10000) + 5000;
  var huhong = Math.floor(Math.random() * 90) + 20;
              a.coins = a.coins + coinduaxe
  ...
  var huhong = Math.floor(Math.random() * 90) + 20;
        ...
        case 'botcoins': {
            ...
            api.unsendMessage(handleReply.messageID)
            await Currencies.decreaseMoney(senderID, parseInt(body));
          var doitien = (parseInt(body)* 80 ) / 100 ;
            a.coins = a.coins + (parseInt(body) * 80 ) / 100 ;
            writeFileSync(this.checkPath(1, senderID), JSON.stringify(a, null, 4));
            return api.sendMessage(`Nạp thành công ${doitien} coins vào game!`, threadID, messageID);
        }
        case 'coinsbot': {
            var a = require(`./duaxe/datauser/${senderID}.json`);
           ...
           var doitien = parseInt(body)
          a.coins = a.coins - (parseInt(body) * 120 ) / 100 ;
            writeFileSync(this.checkPath(1, senderID), JSON.stringify(a, null, 4));
            return api.sendMessage(`Rút thành công ${doitien} coins về tài khoản Mirai!`, threadID, messageID);
        }
    }
