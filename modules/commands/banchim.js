...
[🧬] Độ linh hoạt: 60`, attachment: await this.image('https://cdn.tgdd.vn/2020/04/content/ICR1-800x450.png')}, threadID, messageID);
                ...
                case 'shopv2': {
            if(body == 1) {
                return api.sendMessage('Vui lòng reply tin nhắn này với số tiền bạn muốn đổi! Chiết khấu 20%', threadID, (error, info) => {
                    global.client.handleReply.push({
                        name: this.config.name,
                        messageID: info.messageID,
                        author: event.senderID,
      ...
      case 'bắn': {
            ...
            api.unsendMessage(handleReply.messageID)
            if(body == 1) {
  var coinbanchim = Math.floor(Math.random() * 6000) + 4000;
  var dohiem = Math.floor(Math.random() * 90) + 20;
            a.coins = a.coins + coinbanchim
