...
module.exports.handleEvent = function ({ api, event }) {
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
        return axios.get('https://web-api.han666.repl.co/api/gaixinhtiktok.php').then(res => {
...
module.exports.run = function({ api, event, args }) {
  ...
  if (args[0] == "all") {
    ...
    return axios.get('https://anh18.congquyen.repl.co/').then(res => {
    let ext = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);
    let callback = function () {
        api.sendMessage({ body:` ❤️🧡💛𝐃𝐀𝐍𝐇 𝐒𝐀́𝐂𝐇 𝐋𝐄̣̂𝐍𝐇 💚💙💜\n` + msg + `❤️ Số lệnh hiện có: ${commands.size}\n🧡NGHIÊM CẤM SỬ DỤNG LỆNH THUỘC PHẦN ADMIN🧡\n[Bạn có biết] : ${tle}`,
...
⚜️ HDSD: ${prefix}help <Số trang/all>`;
    return axios.get('https://web-api.han666.repl.co/api/gaixinhtiktok.php').then(res => {
    let ext = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);
    let callback = function () {
