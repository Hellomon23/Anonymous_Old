...
module.exports.config = {
  ...
  commandCategory: "Chat ẩn danh",
  usages: "[random/id/thongtin]",
  cooldowns: 0
}
...
module.exports.run = async function ({ api, event, Users, args }) {
  ...
  switch (args[0]) {
    case 'random':
      ...
      rc.isRandom = true;
      o("Bạn đã chọn chế độ tìm kiếm ngẫu nhiên.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để hiện, 1 để ẩn", (_, i) => global.client.handleReply.push({ type: 'incognitoRootRandomHead', name: this.config.name, author: threadID, messageID: i.messageID, data: rc }));
      break;
    case 'tuchoi':
      var data = require(rcp);
    ...
    case 'dongy':
      ...
      if (data.some(e => e.head == threadID && e.isWaiting)) {
        ...
        o("Bạn đã chấp nhận kết nối với người liên hệ.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để hiện, 1 để ẩn", (_, i) => global.client.handleReply.push({ type: 'incognitoHead', name: this.config.name, author: threadID, messageID: i.messageID, data: ac }))
      }
      break;
    case 'ketthuc':
    case 'thoat':
    ...
    default:
      ...
      o(`Bạn đã yêu cầu ${args[0]} kết nối trò chuyện.\nBạn có muốn ẩn giấu thân phận?\n\nReply với số 0 để hiện, 1 để ẩn`, (_, i) => global.client.handleReply.push({ type: 'incognitoRoot', name: this.config.name, author: threadID, messageID: i.messageID, data: rc }));
      break;
  }
}
...
module.exports.handleReply = async function ({ handleReply, api, event, Users }) {
  ...
  switch (handleReply.type) {
    case 'incognitoRootRandomHead':
      ...
      s((rc.rootIsIncognito ? "Có 1 người liên hệ ẩn danh muốn trò chuyện với bạn" : `Một người tên ${rootName} - ${fbUrl} muốn trò chuyện với bạn.`) + `\nNếu muốn từ chối hãy nhập lệnh: ${global.config.PREFIX}chatandanh tuchoi\nNếu muốn chấp nhận hãy nhập lệnh: ${global.config.PREFIX}chatandanh dongy`, headID, (er, i) => {
    ...
    case 'incognitoHead':
      ...
      if (body == 1) {
        ...
        o(`Bạn đã chọn ẩn giấu thân phận.\nĐể kết thúc cuộc trò chuyện hãy nhập lệnh: ${global.config.PREFIX}chatanhdanh ketthuc`);
      } else if (body == 0) {
        ac.headIsIncognito = false;
        o(`Bạn đã chọn hiện thân phận.\nĐể kết thúc cuộc trò chuyện hãy nhập lệnh: ${global.config.PREFIX}chatandanh ketthuc`);
      }
      ac.isWaiting = false;
      ac.isStarted = true;
    ...
    case 'incognitoRoot':
      ...
      s((rc.rootIsIncognito ? "Có 1 người liên hệ ẩn danh muốn trò chuyện với bạn" : `Một người tên ${rootName} - ${fbUrl} muốn trò chuyện với bạn.`) + `\nNếu muốn từ chối hãy nhập lệnh: ${global.config.PREFIX}chatandanh tuchoi\nNếu muốn chấp nhận hãy nhập lệnh: ${global.config.PREFIX}chatandanh dongy`, rc.head, (er, i) => {
        if (er) o("Đã có lỗi xảy ra khi cố kết nối với người liên hệ.\nVui lòng thử lại sau.");
        else {
