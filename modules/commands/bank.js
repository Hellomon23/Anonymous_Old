    ...
    switch(args[0]) {
      ...
      case '-c': {
        ...
        case '-p': {
            ...
            if(args[1] == 'stk') {
                api.sendMessage('Hoàn tất bước cuối cùng ở tin nhắn chờ', threadID, messageID);
                return api.sendMessage('Vui lòng reply tin nhắn này và nhập mật khẩu Mirai Bank để chuyển tiền!', senderID, (error, info) => 
                    global.client.handleReply.push({
                        name: this.config.name,
                        type: 'paySTK',
                        messageID: info.messageID,
        ...
        case 'send':
        case 'nạp': {
            if(checkBank.status == false) return api.sendMessage('Bạn chưa có tài khoản Mirai Bank!', threadID, messageID);
            ...
            return api.sendMessage(`${res.message.noti} Số dư hiện tại của bạn trong Mirai Bank: ${res.message.money}$`, threadID, messageID)
            break;
    }
        case 'password':
        case 'pw': {
