    ...
    switch (args[0]) {
        ...
        case '-r': {
            ...
            if (!existsSync(pathData)) {
                ...
                return api.sendMessage("🔱 Đăng ký thành công tiến vào hòn đảo", threadID, messageID);
  ...
  const fs = require("fs");
    switch (handleReply.type) {
        case 'steal': {
            ...
            var dataUser = require(`./daorong/datauser/${senderID}`);
        ...
        case 'attack': {
            ...
            return api.sendMessage(`Bạn muốn tấn công vào nơi nào trên đảo!\n1. Chuồng ấp (${u.Island.data.tower}/50)\n2. Chuồng nuôi (${u.Island.data.tree}/50)\n3. Chuồng lai (${u.Island.data.pool}/50)\n4. Môi trường sống (${u.Island.data.pet}/50)\n==============`, threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,
                    author: event.senderID,
...
`, attachment: await this.image('https://imgur.com/8l5FXH7.gif')}, threadID, messageID);
        ...
        case 'đấu': {
            ...
            api.unsendMessage(handleReply.messageID)
            if(body == 1) {
  var coindaorong = Math.floor(Math.random() * 6000) + 5000;
  var luongmau = Math.floor(Math.random() * 90) + 20;
             a.coins = a.coins + coindaorong
                return api.sendMessage({body: `⭐️ THI ĐẤU ⭐️\n[🗺] MAPS: Đảo dung nham.\n[🏆] Chúc Mừng bạn đã chiến thắng đối thủ\n» Giải thưởng của bạn là: ${coindaorong}$\n» Tổn thương: ${luongmau}%`, attachment: await this.image('https://imgur.com/PUCFwqp.jpg')}, threadID, messageID);
        }
            else if(body == 2) {
               var coindaorong = Math.floor(Math.random() * 6000) + 5000;
  var luongmau = Math.floor(Math.random() * 90) + 20;
               a.coins = a.coins + coindaorong
                return api.sendMessage({body: `⭐️ THI ĐẤU ⭐️\n[🗺] MAPS: Đảo tuyết.\n[🏆] Chúc Mừng bạn đã chiến thắng đối thủ\n» Giải thưởng của bạn là: ${coindaorong}$\n» Tổn thương: ${luongmau}%`, attachment: await this.image('https://imgur.com/FLMkCGK.jpg')}, threadID, messageID);
        }
            else if(body == 3) {
   var coindaorong = Math.floor(Math.random() * 6000) + 5000;
  var luongmau = Math.floor(Math.random() * 90) + 20;
               a.coins = a.coins + coindaorong
                return api.sendMessage({body: `⭐️ THI ĐẤU ⭐️\n[🗺] MAPS: Thần Tiên.\n[🏆] Chúc Mừng bạn đã chiến thắng đối thủ\n» Giải thưởng của bạn là: ${coindaorong}$\n» Tổn Thương: ${luongmau}%`, attachment: await this.image('https://imgur.com/k3JyZfJ.jpg')}, threadID, messageID);
            }
        }        
        case 'spinn': {
